...
APPSETTINGS="VardyParty.Desktop/appsettings.json"
LOG="$(mktemp /tmp/vardyparty-desktop-launch.XXXXXX.log)"
...
{
  "$HOME/.dotnet/dotnet" restore VardyParty.Desktop/VardyParty.Desktop.csproj \
    --ignore-failed-sources -p:HomeUiTargetFrameworks=net11.0 \
  && "$HOME/.dotnet/dotnet" run --project VardyParty.Desktop/VardyParty.Desktop.csproj \
...
if [[ "$RC" -ne 0 ]] && grep -q NETSDK1147 "$LOG"; then
  echo
  echo "NETSDK1147: the android workload leaked into the desktop build graph."
