...
REM Do not embed bash -lc "... \"$VAR\"" here: cmd.exe treats \" as closing the
REM quoted string, so $APPSETTINGS never reaches the merge script (exit 2,
REM Usage: /dev/fd/63). The WSL body lives in launch-linux-app.sh (LF).
wsl.exe --cd "%REPO_WSL%" bash -c "tr -d '\r' < scripts/launch-linux-app.sh | bash -s"
