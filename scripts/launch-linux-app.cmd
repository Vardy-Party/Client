...
wsl.exe --cd "%REPO_WSL%" bash -lc "set -euo pipefail; export DISPLAY=${DISPLAY:-:0}; export WAYLAND_DISPLAY=${WAYLAND_DISPLAY:-wayland-0}; export XDG_RUNTIME_DIR=${XDG_RUNTIME_DIR:-/mnt/wslg/runtime-dir}; export USER_SECRETS_ID=543d9e88-b60c-4397-bc9d-c4614b8b1dcb; APPSETTINGS=VardyParty.Desktop/appsettings.json; LOG=$(mktemp /tmp/vardyparty-desktop-launch.XXXXXX.log); restore_appsettings() { git restore -- \"$APPSETTINGS\" 2>/dev/null || true; }; trap restore_appsettings EXIT; bash <(sed 's/\r$//' scripts/merge-appsettings-secrets.sh) \"$APPSETTINGS\"; { $HOME/.dotnet/dotnet restore VardyParty.Desktop/VardyParty.Desktop.csproj --ignore-failed-sources -p:HomeUiTargetFrameworks=net11.0 && $HOME/.dotnet/dotnet run --project VardyParty.Desktop/VardyParty.Desktop.csproj -c Release --no-restore -p:HomeUiTargetFrameworks=net11.0; } 2>&1 | tee $LOG; RC=${PIPESTATUS[0]}; if [ $RC -ne 0 ] && grep -q NETSDK1147 $LOG; then echo; echo 'NETSDK1147: the android workload leaked into the desktop build graph.'; echo 'Remedy: dotnet workload install android'; fi; rm -f $LOG; exit $RC"
