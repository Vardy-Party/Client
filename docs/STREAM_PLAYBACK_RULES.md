...
| Linux host | `VardyParty.Desktop/Services/DesktopVideoPlayerService.cs` | LibVLC facts → same loop; `CurrentStreamIndexChanged` attaches the pool URL (Windows/Android parity). In-player chrome overlays the composited Avalonia `Image` (software-callback vout); standalone fallback keeps the same chrome as companion UI. |
