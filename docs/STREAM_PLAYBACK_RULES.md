...
| Linux host | `VardyParty.Desktop/Services/DesktopVideoPlayerService.cs` | LibVLC facts → same loop; `CurrentStreamIndexChanged` attaches the pool URL (Windows/Android parity). In-player chrome (scores / info / stream count / next) is reserved airspace in `DesktopHomePage`, not over the native child. |
