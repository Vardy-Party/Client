...
| **[homepage-maui-avalonia.md](homepage-maui-avalonia.md)** | One MAUI XAML homepage everywhere; Avalonia draws it on Linux (`VardyParty.HomeUi` + `VardyParty.Linux`). TV focus, clip chain, Crest spin, CI notes. |
...
`HomeUi` (shared XAML), heads `VardyParty` and `VardyParty.Linux`.
