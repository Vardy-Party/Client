    ...
    public class BbcHtmlParserTests
    {
        ...
        public void ParseHtml_ParsesSampleFragment_HT()
        {
            var html = new BbcHtmlBuilder()
                ...
                .AddGame(g => g.WithEventId("s-1").WithHome("Manchester City").WithAway("Chelsea").WithScore(1, 0).WithProgressText("HT"))
                .BuildPage();

            var fixtures = CreateParser().ParseHtml(html);
            var f = fixtures.FirstOrDefault(x => x.Home.Contains("Manchester City", StringComparison.OrdinalIgnoreCase) && x.Away.Contains("Chelsea", StringComparison.OrdinalIgnoreCase));
            Assert.NotNull(f);
        ...
        public void ParseHtml_ParsesFinishedGame_FT()
        {
            var html = new BbcHtmlBuilder()
                .WithLeague("Premier League")
                .AddGame(g => g.WithEventId("s-2").WithHome("TeamA").WithAway("TeamB").WithScore(2, 1).WithProgressText("FT"))
                .BuildPage();
        ...
        public void ParseHtml_ParsesMatchPostponed_VariedText()
        {
            var html = new BbcHtmlBuilder()
                .WithLeague("LeagueZ")
                .AddGame(g => g.WithEventId("s-5").WithHome("A").WithAway("B").WithProgressText("Match postponed due to weather"))
                .BuildPage();
        ...
        public void ParseHtml_Badges_AreExtracted()
        {
            var html = new BbcHtmlBuilder()
                .WithLeague("LeagueB")
                .AddGame(g => g.WithEventId("s-7").WithHome("HB").WithAway("AB").WithHomeBadge("https://example.com/h.svg").WithAwayBadge("https://example.com/a.svg").WithProgressText("Live"))
                .BuildPage();
        ...
        public void ParseHtml_AfterExtraTimeAndPenalties_Parsed()
        {
            var html = new BbcHtmlBuilder()
                ...
                .AddGame(g => g.WithEventId("s-b1ky54ud4knpr8o4m79jfup78")
                    ...
                    .WithScore(1, 1)
                    .WithHomeBadge("https://static.files.bbci.co.uk/core/website/assets/static/sport/football/milton-keynes-dons.0c37c8c1e0.svg")
                    .WithAwayBadge("https://static.files.bbci.co.uk/core/website/assets/static/sport/football/oxford-united.43e728f198.svg")
                    .WithAfterExtraTime(true)
                    .WithPenaltyResult("Oxford United", 4, 3))
                .BuildPage();

            var fixtures = CreateParser().ParseHtml(html);
            var f = fixtures.FirstOrDefault(x => x.Home.Contains("Milton Keynes", StringComparison.OrdinalIgnoreCase) && x.Away.Contains("Oxford United", StringComparison.OrdinalIgnoreCase));
            Assert.NotNull(f);
        ...
        public void ParseHtml_InitialJson_Postponed_Fallback()
        {
            ...
            mockParser.Setup(p => p.BuildEventStatusMapStreaming(It.IsAny<string>(), It.IsAny<System.Threading.CancellationToken>()))
        ...
        public void ParseHtml_ParsesManyGames_NoCrash()
        {
            ...
            for (int i = 0; i < 50; i++)
            {
                builder.AddGame(g => g.WithEventId($"s-{i}").WithHome($"Home{i}").WithAway($"Away{i}").WithScore(i % 3, (i + 1) % 3).WithProgressText(i % 2 == 0 ? "Live" : string.Empty));
            }
        ...
        public void ParseHtml_ParsesInExtraTime_ET_NotFinished()
        {
            var html = new BbcHtmlBuilder()
                .WithLeague("Premier League")
                .AddGame(g => g.WithEventId("s-et1").WithHome("Newcastle").WithAway("Bournemouth").WithScore(2, 2).WithInExtraTime(92))
                .BuildPage();
        ...
        {
            var homeBadge = "https://static.files.bbci.co.uk/core/website/assets/static/sport/football/newcastle-united.7daf913814.svg";
            var awayBadge = "https://static.files.bbci.co.uk/core/website/assets/static/sport/football/afc-bournemouth.3e0ae7da8e.svg";
            ...
            var fixtures = CreateParser().ParseHtml(html);
            var f = fixtures.FirstOrDefault(x => x.Home.Contains("Newcastle", StringComparison.OrdinalIgnoreCase) && x.Away.Contains("Bournemouth", StringComparison.OrdinalIgnoreCase));
            Assert.NotNull(f);
        ...
        public void ParseHtml_ExtractsTeamLogos_Lazio_Como()
        {
            ...
            Assert.Equal("https://static.files.bbci.co.uk/core/website/assets/static/sport/football/lazio.8fc1f19371.svg", f.HomeBadgeUrl);
            Assert.Equal("https://static.files.bbci.co.uk/core/website/assets/static/sport/football/como.57ce7c985f.webp", f.AwayBadgeUrl);
        }
        ...
        public void ParseHtml_ExtractsTeamLogos_WithBuilder()
        {
            ...
            var lazioLogo = "https://static.files.bbci.co.uk/core/website/assets/static/sport/football/lazio.8fc1f19371.svg";
        ...
        public void BbcHtmlBuilder_FullTimeGame_ProducesCorrectMarkup()
        {
            ...
            var interMilanBadge = "https://static.files.bbci.co.uk/core/website/assets/static/sport/football/inter-milan.209b8285b0.svg";
            var arsenalBadge = "https://static.files.bbci.co.uk/core/website/assets/static/sport/football/arsenal.5be7ff54ce.svg";
        ...
        public void ParseHtml_ComplexStatusText_ParsesMinutesAndSpecials()
        {
            ...
            var html = new BbcHtmlBuilder()
                .WithLeague("LeagueX")
                .AddGame(g => g.WithEventId("s-inj").WithHome("InjTeam").WithAway("Opp").WithScore(1,1).WithProgressText("90+3"))
                .AddGame(g => g.WithEventId("s-et").WithHome("ETTeam").WithAway("Opp").WithScore(2,2).WithProgressText("100' ET"))
                .AddGame(g => g.WithEventId("s-pen").WithHome("PenTeam").WithAway("Opp").WithScore(0,0).WithProgressText("Penalties 5-4"))
                .BuildPage();
